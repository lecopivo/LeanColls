...
instance [LawfulIndexType ι] : LawfulIndexed (FixSize C ι) ι τ where
  get_ofFn f := by sorry -- simp [Indexed.ofFn, GetElem'.get, getElem]; sorry
  get_set_eq := by sorry
    -- intros;
    -- simp [Indexed.set, GetElem'.get, getElem, -GetElem'.get_normalize]
